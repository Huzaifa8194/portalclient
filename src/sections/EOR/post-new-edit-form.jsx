...
import { MenuItem, Stepper, Step, StepLabel, Box, Button } from "@mui/material"
...
export function PostNewEditForm({ currentPost }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      complianceDocuments: currentPost?.complianceDocuments || null,
    }),
    [currentPost],
  )
        ...
        <Stack spacing={1.5}>
          ...
          <Field.Upload
            ...
            onDrop={(acceptedFiles) => {
              const newFiles = [...(values.employmentContract || []), ...acceptedFiles]
              setValue("employmentContract", newFiles)
            }}
            onRemove={(file) => {
              const filteredFiles = values.employmentContract.filter((f) => f !== file)
              setValue("employmentContract", filteredFiles)
            }}
            onRemoveAll={() => {
              setValue("employmentContract", [])
            }}
          />
        </Stack>
  ...
  const renderAdditionalInformation = (
    <Card sx={{ border: "1px solid", borderColor: "divider", boxShadow: "none", "& .MuiCardHeader-root": { p: 2 } }}>
      ...
      <Stack spacing={3} sx={{ p: 3 }}>
        ...
        <Field.Upload name="complianceDocuments" label="Compliance Documents" multiple />
      </Stack>
    </Card>
  )
