...
export function JobItem({ job, onEdit, onDelete }) {
  ...
  // Use useMemo to compute these values only when job.nationality changes
  const { flagUrl, displayCountry } = useMemo(() => {
    try {
      const rawCountry = (job.nationality || "").toLowerCase().trim()
      const countryData = countries[rawCountry] || {
        code: rawCountry.slice(0, 2),
        display: job.nationality || "N/A",
      }
      ...
      return {
        displayCountry: job.nationality || "N/A",
  ...
  return (
    <>
      ...
      >
        ...
        <Box display="grid" gridTemplateColumns="repeat(2, 1fr)" sx={{ py: 3, typography: "subtitle1", gap: 1 }}>
          <div>
      ...
      <CoApplicantProfile applicantId={job.id} open={isProfileOpen} onClose={handleCloseProfile} />
    </>
  )
}
